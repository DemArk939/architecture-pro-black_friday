...
async def root():
    ...
    for collection_name in collection_names:
        ...
        # Подсчет общего числа документов в коллекции
        total_docs = await collection.count_documents({})

        # Получение распределения документов по шардам
        aggregation_result = await collection.aggregate([{"$collStats": {"storageStats": {}}}]).to_list(length=None)
        distribution = []
        for result in aggregation_result:
            stats = result.get('storageStats', {})  # Получаем storageStats
            shard_id = result.get('shard')  # Проверяем наличие поля "shard"
            docs_on_shard = stats.get('count', 0)  # Проверяем наличие поля "count"
            if shard_id is not None:
                distribution.append({
                    "shard_id": shard_id,
                    "docs_on_shard": docs_on_shard
                })

        collections[collection_name] = {
            "total_documents": total_docs,
            "distribution_by_shards": distribution
        }
    try:
